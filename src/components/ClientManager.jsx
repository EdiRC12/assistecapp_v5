...
import { 
    X, Plus, Edit2, Trash2, Search, MapPin, Phone, Save, Briefcase, 
    ChevronLeft, ShieldCheck, MessageSquare, Users, Calendar, 
    Filter, CalendarClock, Zap, Crown, Award, Loader2
} from 'lucide-react';
import { supabase } from '../supabaseClient';
import { normalizeState } from '../utils/addressUtils';

const ClientManager = ({ isOpen, onClose, currentUser, initialData, notifySuccess, notifyError, initialViewMode = 'MANAGE' }) => {
    const [clients, setClients] = useState([]);
    const [loading, setLoading] = useState(false);
    const [searchTerm, setSearchTerm] = useState('');
    const [isEditing, setIsEditing] = useState(false);
    
    // Modo de Visualização: 'MANAGE' (Padrão) ou 'QUICK_SLA' (Gestão Rápida por Estado/Categoria)
    const [viewMode, setViewMode] = useState(initialViewMode);

    // Filtros do Modo Gestão Rápida de SLA
    const [filterState, setFilterState] = useState('TODOS');
    const [filterCategory, setFilterCategory] = useState('TODAS');
    const [updatingClientId, setUpdatingClientId] = useState(null);

    // Meta visitation states (Modal legado)
    ...
    useEffect(() => {
        if (isOpen && currentUser) {
            setViewMode(initialViewMode || 'MANAGE');
            fetchClients();
            if (initialData) {
    ...
    }, [isOpen, currentUser, initialData, initialViewMode]);
    ...
    // Atualização rápida de SLA inline
    const handleQuickUpdateSLA = async (clientObj, newFreqVal) => {
        ...
        setUpdatingClientId(clientObj.id);
        ...
        try {
            ...
            // Atualizar lista local de forma reativa
            setClients(prevClients => 
                prevClients.map(c => 
                    c.id === clientObj.id 
                        ? { ...c, visit_frequency_months: finalFreq, visit_frequency_value: finalFreq }
                        : c
                )
            );
            ...
            if (notifySuccess) {
                notifySuccess(
                    'Meta Atualizada',
                    `${clientObj.name}: Novo prazo configurado para ${months > 0 ? `a cada ${months} meses` : 'Sem Meta'}.`
                );
            }
        } catch (error) {
    ...
    // Atualização rápida da classificação (Ouro/Prata/Bronze) inline
    const handleQuickUpdateClassification = async (clientObj, newTier) => {
        if (!clientObj) return;
        setUpdatingClientId(clientObj.id);
        ...
        try {
            ...
            setClients(prevClients => 
                prevClients.map(c => 
                    c.id === clientObj.id ? { ...c, classification: newTier } : c
                )
            );

            if (notifySuccess) {
                notifySuccess('Categoria Atualizada', `${clientObj.name} agora é ${newTier}.`);
            }
        } catch (error) {
            console.error('Erro ao atualizar categoria do cliente:', error);
            if (notifyError) notifyError('Erro', error.message);
        } finally {
            setUpdatingClientId(null);
        }
    };
    ...
    const uniqueStates = useMemo(() => {
        ...
        clients.forEach(c => {
            if (c.state && c.state.trim()) {
                set.add(c.state.trim().toUpperCase());
            }
        });
        return Array.from(set).sort();
    }, [clients]);

    // Filtragem de clientes para o Modo Tradicional e para a Gestão Rápida
    const filteredClients = useMemo(() => {
        return clients
            .filter(c => {
                ...
                const matchesState = filterState === 'TODOS' || (c.state && c.state.trim().toUpperCase() === filterState);
                
                const clientClass = (c.classification || 'BRONZE').toUpperCase();
                const matchesCategory = filterCategory === 'TODAS' || clientClass === filterCategory;

                return matchesSearch && matchesState && matchesCategory;
            })
            .sort(sortAlphaNum);
    }, [clients, searchTerm, filterState, filterCategory]);
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-0 md:p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white md:rounded-3xl shadow-2xl w-full max-w-5xl h-full md:h-[85vh] flex flex-col overflow-hidden">
                
                {/* ── HEADER COM SELETOR DE MODO ────────────────────────────────── */}
                <div className="p-4 md:p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 text-white shrink-0">
                    <div className="flex items-center gap-3">
                        {isEditing && (
                            <button onClick={() => setIsEditing(false)} className="md:hidden p-2 -ml-2 hover:bg-slate-800 rounded-lg text-indigo-400 transition-colors">
                                <ChevronLeft size={24} />
                            </button>
                        )}
                        <div className="bg-indigo-500/20 border border-indigo-400/30 p-2.5 rounded-2xl text-indigo-400">
                            <Users size={22} />
                        </div>
                        <div>
                            <h2 className="text-base md:text-lg font-black text-white">
                                Central de Clientes & Prazos
                            </h2>
                            <p className="text-xs text-slate-400">
                                Gerencie contatos, classificações e prazos de visitas
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-2">
                        <div className="flex bg-slate-800 p-1 rounded-xl border border-white/10">
                            <button
                                onClick={() => { setViewMode('MANAGE'); setIsEditing(false); }}
                            ...
                            >
                                <Users size={14} />
                                <span>Lista & Cadastro</span>
                            </button>

                            <button
                                onClick={() => setViewMode('QUICK_SLA')}
                                className={`px-3 py-1.5 rounded-lg text-xs font-black transition-all flex items-center gap-1.5 ${
                                    viewMode === 'QUICK_SLA'
                                        ? 'bg-indigo-600 text-white shadow'
                                        : 'text-slate-400 hover:text-white'
                            ...
                            >
                                ...
                                <span>Gestão Rápida de Prazos</span>
                            </button>
                        </div>
                ...
                {/* ── MODO 2: GESTÃO RÁPIDA DE PRAZOS POR ESTADO & CATEGORIA ───────── */}
                {viewMode === 'QUICK_SLA' ? (
                    <div className="flex-1 flex flex-col overflow-hidden bg-slate-50">
                        
                        {/* BARRA DE FILTROS RÁPIDOS */}
                        <div className="p-4 bg-white border-b border-slate-200 flex flex-col md:flex-row items-center justify-between gap-4 shrink-0 shadow-2xs">
                            <div className="relative flex-1 w-full">
                                ...
                                <input
                                    ...
                                    className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                />
                            </div>
                            ...
                            <div className="flex flex-wrap items-center gap-3 w-full md:w-auto shrink-0 justify-end">
                                ...
                                <div className="flex items-center gap-1.5">
                                    <label className="text-[11px] font-black uppercase text-slate-400">Estado (UF):</label>
                                    <select
                                        value={filterState}
                                        onChange={(e) => setFilterState(e.target.value)}
                                        className="bg-slate-100 border border-slate-200 text-slate-800 text-xs font-black px-3 py-2 rounded-xl outline-none cursor-pointer hover:bg-slate-200"
                                    >
                        ...
                        {/* GRADE DE CARDS ÁGEIS DE CLIENTES */}
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6">
                            {filteredClients.length === 0 ? (
                                <div className="text-center py-16 text-slate-400 space-y-3">
                                    <Users size={48} className="mx-auto text-slate-300" />
                                    <h3 className="text-base font-black text-slate-700">Nenhum cliente encontrado</h3>
                                    <p className="text-xs text-slate-400 max-w-sm mx-auto">
                                        Tente alterar os filtros de Estado (UF) ou Categoria no topo.
                                    </p>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {filteredClients.map(client => {
                                        ...
                                        return (
                                            <div
                                                key={client.id}
                                                className={`bg-white rounded-2xl p-4 border shadow-2xs hover:shadow-md transition-all space-y-3 relative overflow-hidden ${
                                                    rawClass === 'OURO' ? 'border-amber-300 bg-gradient-to-br from-amber-50/20 to-white' :
                                                    rawClass === 'PRATA' ? 'border-slate-300 bg-gradient-to-br from-slate-50/30 to-white' :
                                                    'border-slate-200'
                                                }`}
                                            >
                                                {/* Header do Card: Nome e Categoria */}
                                                <div className="flex items-start justify-between gap-2">
                                                    <div className="min-w-0 flex-1">
                                                        <h4 className="font-black text-slate-800 text-sm truncate" title={client.name}>
                                                            {client.name}
                                                        </h4>
                                                        <p className="text-[11px] text-slate-500 flex items-center gap-1 mt-0.5">
                                                            <MapPin size={11} className="text-slate-400 shrink-0" />
                                                            {client.city || 'Sem cidade'} - <strong className="text-slate-700">{client.state || 'UF'}</strong>
                                                        </p>
                                                    </div>
                                                    ...
                                                    <select
                                                        value={rawClass}
                                                        onChange={(e) => handleQuickUpdateClassification(client, e.target.value)}
                                                        disabled={isUpdating}
                                                        className={`text-[10px] font-black uppercase px-2 py-1 rounded-lg border outline-none cursor-pointer transition-all ${
                                                            rawClass === 'OURO' ? 'bg-amber-100 text-amber-900 border-amber-300' :
                                                            rawClass === 'PRATA' ? 'bg-slate-200 text-slate-800 border-slate-300' :
                                                ...
                                                {/* Seletor Inline de Meta de SLA */}
                                                <div className="pt-2 border-t border-slate-100 flex items-center justify-between gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-200/60">
                                                    ...
                                                    {isUpdating ? (
                                                        ...
                                                        <select
                                                            ...
                                                            className="bg-white border border-indigo-200 hover:border-indigo-400 font-bold text-indigo-900 text-xs rounded-lg px-2 py-1 outline-none cursor-pointer shadow-2xs"
                                                        >
                                                            <option value="0">Sem Meta / Libre</option>
                    ...
                    /* ── MODO 1: CADASTRO TRADICIONAL & LISTA COMPLETA ──────────────── */
                    <div className="flex-1 flex overflow-hidden">
                        ...
                        <div className={`${isEditing ? 'hidden md:flex' : 'flex'} w-full md:w-1/2 border-r border-slate-100 flex-col bg-white overflow-hidden`}>
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-2">
                                <button onClick={() => { resetForm(); setIsEditing(true); }} className="w-full py-3 border-2 border-dashed border-slate-200 rounded-xl text-slate-400 font-bold text-sm flex items-center justify-center gap-2 hover:border-indigo-300 hover:text-indigo-600 hover:bg-indigo-50 transition-all mb-4">
                                    <Plus size={18} /> CADASTRAR NOVO CLIENTE
                                </button>

                                {filteredClients.map(client => (
                                    <div key={client.id} className={`p-4 rounded-xl border transition-all text-left group ${formData.id === client.id ? 'border-indigo-500 bg-indigo-50/50' : 'border-slate-100 hover:border-slate-300 hover:bg-slate-50'}`}>
                                ...
                                <form onSubmit={handleSave} className="w-full md:max-w-sm bg-white p-6 md:rounded-2xl md:shadow-sm md:border md:border-slate-200 animate-in zoom-in-95 duration-200 my-0 md:my-auto min-h-full md:min-h-0">
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500 transition-all font-bold" placeholder="Ex: Cliente 1234" required />
                                        </div>

                                        <div>
                                            <label className="block text-xs font-black text-slate-400 uppercase mb-1.5">Telefone Geral (Recepção)</label>
                                            <input type="text" value={formData.main_phone} onChange={(e) => setFormData({ ...formData, main_phone: e.target.value })} className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500 transition-all" placeholder="(00) 0000-0000" />
                                        </div>
                                        ...
                                        <div className="grid grid-cols-3 gap-3">
                                            <div className="col-span-2">
                                                ...
                                                <input type="text" value={formData.street} onChange={(e) => setFormData({ ...formData, street: e.target.value })} className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Rua..." />
                                            </div>
                                            <div>
                                                <label className="block text-xs font-black text-slate-400 uppercase mb-1.5">Nº</label>
                                                <input type="text" value={formData.number} onChange={(e) => setFormData({ ...formData, number: e.target.value })} className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" placeholder="00" />
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            <div>
                                                ...
                                                <input type="text" value={formData.neighborhood} onChange={(e) => setFormData({ ...formData, neighborhood: e.target.value })} className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Bairro..." />
                                            </div>
                                            <div className="grid grid-cols-3 gap-2">
                                                <div className="col-span-2">
                                                    <label className="block text-xs font-black text-slate-400 uppercase mb-1.5">Cidade</label>
                                                    <input type="text" value={formData.city} onChange={(e) => setFormData({ ...formData, city: e.target.value })} className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Cidade" />
                                                </div>
                                                <div>
                                                    <label className="block text-xs font-black text-slate-400 uppercase mb-1.5">UF</label>
                                                    <input type="text" value={formData.state} onChange={(e) => setFormData({ ...formData, state: e.target.value })} className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" placeholder="UF" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex gap-3 mt-8">
                                        ...
                                        <button type="submit" disabled={loading} className="flex-1 py-2.5 px-4 rounded-xl bg-indigo-600 text-white font-black text-sm shadow-lg hover:bg-indigo-700 transition-all disabled:opacity-50 flex items-center justify-center gap-2 uppercase">
